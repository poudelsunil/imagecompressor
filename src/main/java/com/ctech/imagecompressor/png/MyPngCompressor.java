...
public class MyPngCompressor {
     ...
     */
    public MyPngCompressor(boolean compress) {
        this.compress = compress;
    }

    public byte[] compress(final byte[] image) throws IOException {
        return compress8Bit(image);
    }
    ...
    private byte[] compress8Bit(byte[] originalImageByte) throws IOException {

        BufferedImage originalImage;
        try {
            originalImage = ImageIO.read(new ByteArrayInputStream(originalImageByte));
        } catch (IOException e) {
        ...
        final int bh = originalImage.getHeight();
        final int bw = originalImage.getWidth();
        final int dim = bh * bw;
        ...
        switch (type) {
            ...
            case BufferedImage.TYPE_INT_BGR:
                intPixels = ((DataBufferInt) originalImage.getRaster().getDataBuffer()).getData();
                rgb = new int[bh][bw];
                for (int y = 0; y < bh; y++) {
                    tempArr = rgb[y];
                    for (int x = 0; x < bw; x++) {
            ...
            case BufferedImage.TYPE_INT_RGB:
                ...
                break;
            default:
                return compressNormal(originalImage);
        }
        ...
        outputStream.write(chunk.getData());
        outputStream.write(chunk.getCRCValue());
        byte[] compressedByte = outputStream.toByteArray();
        outputStream.flush();
        outputStream.close();
        return compressedByte;
    }

    private byte[] compressNormal(BufferedImage originalImage) throws IOException {
        ...
        if(isIndexed && bitDepth!=8){
            pixels = getIndexedPaletteData(originalImage);
        }else{
            pixels = getPixelData(originalImage, bitDepth, nComp, bw, bh);
        }
